...
config/claude-automic-vault  optional exact "on" flag enabling fail-closed Automic Vault injection for Firstmate-launched Claude workers only; LOCAL, gitignored, inherited only by validated local secondmate homes on the same Mac without copying the token; see docs/configuration.md "Claude authentication through Automic Vault"
...
config/orchestra-dashboard  optional absolute Orchestra checkout path; each actionable watcher wake and heartbeat starts a detached best-effort refresh, with coalescing owned by Orchestra; LOCAL, gitignored, and not inherited; see docs/configuration.md "Orchestra live-board refresh"
...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  projects.md        thin fleet navigation registry recording each project's standing delivery mode, merge authority, and lifecycle posture; firstmate-private, with lifecycle writes owned by fm-project-posture.sh and mechanical delivery reads by fm-project-mode.sh (section 6)
...
Load `project-management` before adding, creating, removing, initializing, parking, un-parking, or archiving a project.
...
- `project-management` - load before adding, creating, removing, initializing, parking, un-parking, or archiving a project.
  Cloning or registering a project is add intake and uses the same trigger.
