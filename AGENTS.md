...
config/claude-automic-vault  optional exact "on" flag enabling fail-closed Automic Vault injection for Firstmate-launched Claude workers only; LOCAL, gitignored, inherited by secondmate homes without copying the token; see docs/configuration.md "Claude authentication through Automic Vault"
