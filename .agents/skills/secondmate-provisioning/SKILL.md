...
Before any local convergence can inherit an enabled Claude Automic Vault flag, the validated destination home must contain the exact tracked authentication-owner version marker required by the primary; an absent, unsafe, or incompatible marker blocks propagation, and the separate launch check blocks launch, until tracked code is synchronized.
