...
The same placement-specific launch and deferred bootstrap sweep also propagate the primary's declared inherited local material: local homes receive `config/crew-dispatch.json`, `config/crew-harness`, `config/backlog-backend`, `config/backend`, `config/herdr-presentation-spaces`, `config/startup-memory-budget`, `config/claude-automic-vault`, `config/trace-context`, and the one shared captain-preference file `data/captain-shared.md`, while remote homes receive the remotely eligible subset defined by the inheritance owner.
...
Inherited `config/claude-automic-vault` carries only the fail-closed opt-in between validated local homes on the same Mac; the Automic Vault token is never copied, and remote inheritance omits and removes this machine-local flag.
Before any local convergence can inherit an enabled Claude Automic Vault flag, the validated destination revision must track the authentication-owner version marker at or above the primary's required contract and every owner implementation file must match that revision; absent, stale, untracked, substituted, unsafe, or inconclusive proof blocks propagation, and the separate launch check blocks launch, until tracked code is synchronized.
...
A remote home receives the remotely eligible allowlisted bytes through `fm-remote-inherit.sh`, converges every machine-local-only item absent, and gets one marked re-read instruction after a changed transfer.
