...
make_fake_root() {
  ...
  # fm-session-lock-lib.sh: the tmux adapter sources it since the upstream
  # session-lock reconcile; a missing sourced sibling aborts teardown mid-kill
  # under set -e even inside a guarded call.
  ...
  # fm-landed-lib.sh: teardown sources the shared landed-work predicates.
