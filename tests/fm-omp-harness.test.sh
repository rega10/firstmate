...
test_detection_anchored_name_and_marker_precedence() {
  local bin out no_harness_chain
  ...
  # Keep the invoking test harness beyond fm-harness.sh's bounded ancestry walk,
  # so this fixture proves the claimed marker-only path under every harness.
  # shellcheck disable=SC2016 # the quoted body expands inside each nested shell
  no_harness_chain='if [ "$1" -gt 0 ]; then bash -c "$0" "$0" "$(( $1 - 1 ))" "$2"; else "$2"; fi'
  out=$(env -u PI_CODING_AGENT -u CURSOR_AGENT -u CURSOR_INVOKED_AS CLAUDECODE=1 FM_OMP_HARNESS=omp \
    bash -c "$no_harness_chain" "$no_harness_chain" 10 "$HARNESS")
