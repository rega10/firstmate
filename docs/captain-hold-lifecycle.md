...
For that replacement, the command acquires every possible prior-entry control lock in lexical order before the origin metadata lock and keeps those task locks through the settlement checks and empty-inventory publication, so a concurrent re-hold cannot cross the commit.
A backlog read failure is never classified as absence and leaves the prior inventory intact.
...
The completion coverage also proves that `--none` retires a prior inventory after its captain-held task is pruned by zero-Done retention, refuses and preserves an inventory whose task is still an open captain call, serializes empty-inventory publication against a concurrent re-hold, and fails closed without changing the inventory when the backlog cannot be read.
