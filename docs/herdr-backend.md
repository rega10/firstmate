...
| Kill | `herdr pane close <pane>` | Closing a tab's only (root) pane also closes the tab - no separate tab-close call is needed for this adapter's one-pane-per-tab shape. General endpoint cleanup remains best-effort, but task teardown follows the stronger confirmed-close contract in "Default workspace lifecycle" above. Teardown only targets the task's recorded pane/tab, never the workspace, but closing a workspace's last tab deletes the workspace as a verified Herdr side effect, so a home's own workspace persists only while at least one task tab remains. |
...
Verified before/after in an isolated `fm-lab-*` session via `bin/fm-herdr-lab.sh` (default session tripwire identical); unit coverage in `tests/fm-backend-herdr.test.sh` (`test_events_capable_*`).
