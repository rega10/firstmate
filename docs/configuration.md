...
While the opt-in is valid, a raw launch is conservatively refused when semantic command classification is unavailable, encounters unsupported compound shell grammar, or cannot statically resolve every executable command position, including when a command may be unrelated to Claude.
