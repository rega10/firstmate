...
An opted-in raw Claude launch, including one reached through leading assignments, supported `env` options, an `env` split string, or a literal `sh`, `bash`, or `zsh -c` payload, is refused before endpoint creation because only the verified Claude harness template carries the required injection boundary.
