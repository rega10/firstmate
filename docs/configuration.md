...
Every enabled Firstmate Claude launch and relaunch resolves `av` before endpoint creation, verifies the canonical native executable under `.local/share/claude/versions` against Anthropic's HTTPS release-manifest checksum, and pins the attested absolute path in the launch command.
