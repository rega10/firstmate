...
Every enabled Firstmate Claude launch and relaunch resolves `av` before endpoint creation, positively identifies `claude` as the canonical native executable under `.local/share/claude/versions`, and pins both absolute paths in the launch command.
