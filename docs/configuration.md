...
An opted-in raw Claude launch, including one reached through leading assignments, supported `env` options, or an `env` split string, is refused before endpoint creation because only the verified Claude harness template carries the required injection boundary.
