...
Contribution coverage and actor counts still include all owned obligations, while `contributions.lifecycle_suppressed` and an `omitted[]` line disclose archived and parked calls removed from the contribution call list.
Secondmate contribution calls with missing project identity remain visible; presentation never guesses from the parent's project registry.
...
Claude Code additionally requires the per-session `CLAUDE_CODE_ENABLE_FUNCTION_HOOKS=1` opt-in; [Claude Code Calm activation](calm.md#claude-code) owns the experimental mod gate and loading requirements.
...
A legacy primary-home Herdr record that predates that binding can be restored only through the guarded evidence-gated `bin/fm-herdr-legacy-repair.sh` ([`herdr-backend.md`](herdr-backend.md#legacy-record-repair)); ordinary cleanup and control never repair a binding implicitly.
...
Missing or locked Vault state, denied Secret Gate access, a missing secret, an invalid or revoked token, an unqualified version, unsupported CLI surfaces, a timeout, and inconclusive or conflicting authentication all block the launch with a redacted action.
...
That startup refresh fetches only the clones named by a backlog item that is In flight or Queued, including held and blocked items, plus clones named by live task metadata, because each private-clone fetch can cost a separate credential approval; [`fm-fleet-sync.sh`'s header](../bin/fm-fleet-sync.sh) owns the selection.
A repo-less queued item is refreshed by `fm-spawn.sh` when it is dispatched, which is the normal on-demand refresh for work not yet started.
