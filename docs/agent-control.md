...
5. **Launch the replacement** through its single owner, `bin/fm-spawn.sh --relaunch`, which adopts the recorded endpoint and worktree instead of creating either, clears the previous harness's per-task wiring, and arms a fresh busy generation.
   An already-authenticated PR merge poll remains authenticated through replacement publication, including when trace-context propagation updates the task metadata.
...
- `tests/fm-control-relaunch.test.sh` - the relaunch transaction: identity preservation, armed PR-poll authentication with and without trace propagation, harness switching, the progress note, checkpoint refusals, and rollback after a failed launch.
