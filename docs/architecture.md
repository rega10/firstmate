...
`data/projects.md` records each project's standing delivery mode, optional `+yolo` merge flag, and orthogonal lifecycle posture as the captain's defaults and context for intake, including the conditional `no-mistakes-prod-only` policy; a ship spawn that drops below the registered delivery rigor prints a deviation notice and continues.
