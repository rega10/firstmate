...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is neither provably working nor parked, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working and whose task is not parked, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
A task admitted by the shared parked predicate in `bin/fm-classify-lib.sh` is the one exception to that bound: its idle pane takes the bounded `FM_PAUSE_RESURFACE_SECS` recheck instead of a wedge escalation, regardless of whether its agent is live or has exited, while an unhandled steering-inbox record keeps the inbox ladder in charge.
Lifting the declaration restores the unchanged busy-pane wedge path, while a pane that is no longer busy returns to the same shared parked-task classification.
