...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is neither provably working nor parked, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working and whose task is not parked, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, parked external waits and captain-held tasks that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
A task admitted by the shared parked predicate in `bin/fm-classify-lib.sh` is the one exception after that bound: it takes the bounded `FM_PAUSE_RESURFACE_SECS` recheck instead of a wedge escalation, while an unhandled steering-inbox record keeps the inbox ladder in charge.
Lifting the declaration restores the unchanged busy-pane wedge path, while an idle pane remains on the same parked cadence until the declaration or active captain hold is removed.
...
Idle-pane classification, including the parked-task exception and its steering-inbox handoff, is owned by `task_is_parked` in `bin/fm-classify-lib.sh`; the normal watcher and away-mode daemon consume that one predicate.
Fresh nonparked stale panes use the same current-state read before trusting the status log, so an active run or a proven busy worker outranks an old captain-relevant status-log line left behind before validation.
...
A declared external wait, verified captain-held transfer, or active captain-held backlog task trades that silence for one bounded recheck per pause window, naming which human the wait is on, so neither a forgotten pause nor a forgotten hold can remain invisible indefinitely.
...
The watcher and daemon share `bin/fm-classify-lib.sh` for captain-relevant status verbs, declared-wait vocabulary, the inbox-aware `task_is_parked` predicate, and status-scan primitives.
...
The always-on watcher and away-mode daemon both consume that parked predicate before entering the idle-pane wedge ladder, while the daemon maintains the same distinct long recheck cadence.
...
The daemon escalates captain-relevant events, plus the bounded parked-task recheck owned by `bin/fm-classify-lib.sh` when an idle wait remains declared, naming which human that wait is on, as one batched, single-line digest using the canonical `away-supervisor` kind from `bin/fm-operational-input.sh` so firstmate can distinguish it structurally from real messages.
...
`data/projects.md` records each project's standing delivery mode, optional `+yolo` merge flag, and orthogonal lifecycle posture as the captain's defaults and context for intake; a ship spawn that drops below the registered delivery rigor prints a deviation notice and continues.
...
`bin/fm-project-posture.sh` is the only lifecycle-token writer and owns its once-only dated-expiry wake, while the fleet snapshot is the shared structured read surface for Bearings and downstream boards.
