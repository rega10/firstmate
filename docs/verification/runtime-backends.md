...
The confirmed-close teardown contract was rerun on 2026-07-27 with a stubbed Herdr CLI so no live session lifecycle was touched:

```sh
bash tests/fm-teardown.test.sh
```

Observed fail-closed guarantees:

```text
ok - herdr projection teardown refuses and preserves state when exact-pane close is unconfirmed
ok - herdr projection teardown refuses and preserves state while the exact pane remains readable
ok - herdr close failure refuses teardown and preserves authoritative task state
ok - forced secondmate cleanup preserves Herdr child state when its target is missing
ok - forced secondmate projected-child cleanup preserves focus and fails closed
```
...
On 2026-07-24, the capability probe was checked against Herdr 0.7.4 protocol 16 on macOS aarch64 with stock Bash 3.2.57.
The old early-exit `grep -Fq` pipeline closed while the shell was still writing the roughly 220 KB API schema and emitted terminal-only broken-pipe noise even though the capability verdict was successful.
...
Before the in-process schema matching fix:
...
The lab teardown accepted the default-session tripwire, and `tests/fm-backend-herdr.test.sh` pins the capable, missing-needle, and no-early-exit-pipeline cases.
...
On 2026-07-23, metadata-publication failure handling was verified on macOS Darwin 25.5.0 with stock GNU Bash 3.2.57 and GNU Bash 5.2.0.

```sh
/bin/bash tests/fm-backend-orca.test.sh
PATH="$PWD/.tasktmp/bash-check/install/bin:$PATH" \
  .tasktmp/bash-check/install/bin/bash tests/fm-backend-orca.test.sh
```

Before the explicit write-status check, the stock-Bash run stopped at:

```text
not ok - Orca spawn should fail when metadata cannot be written
```

After the fix, both commands exited 0 and included:
