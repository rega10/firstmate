...
It also proves the final Claude exec receives exactly one `--settings` value, that this authoritative value disables `apiKeyHelper` and matching authentication environment settings while preserving `feedbackDrafts: off` and the attribution-off policy, and that no worker-template settings survive into the exec arguments.
The same captured-launch regression covers the inherited `config/claude-permission-mode=auto` selection, proving it reaches the final exec without bypass permissions or an unconditional Bash allow grant.
