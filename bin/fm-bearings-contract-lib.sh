  ...
  jq -e . <<'CONTRACT' || { echo "fm-bearings-snapshot: contract JSON is invalid" >&2; return 1; }
{
  ...
  "enums": {
    ...
    "secondmate_provenance": ["structured-home", "structured-home-cache", "parent-event-fallback", "registered-table", "unknown"],
    "secondmate_freshness": ["fresh", "cached", "historical-event", "unavailable", "unknown"],
  ...
  "surfaces": {
    ...
    "secondmate_reconcile": {
      "type": "array",
      "presence": "always",
      "notes": "One row per registered secondmate whose canonical snapshot carries a reconcile-inventory request; empty when none is pending.",
      "fields": {
        "id": {"type": "string"},
        "spawn_gen": {"type": "number", "notes": "Recorded spawn generation, or null when unknown."},
        "host": {"type": "string", "notes": "Remote host, or null for a local secondmate."},
        "kind": {"type": "string", "notes": "Reconcile-inventory kind, or null."},
        "ids": {"type": "array", "notes": "Sorted string ids the reconcile inventory names."}
      }
    },
    "decisions_open": {
      "type": "array",
