...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness backlog-backend backend herdr-presentation-spaces startup-memory-budget claude-automic-vault trace-context launch-env-allowlist claude-permission-mode}"
...
propagate_inheritable_config() {
  local src_config=$1 dest_config=$2 item src dest source_present reason rc dest_home src_state dest_state
