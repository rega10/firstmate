...
Default fields: schema, home, generated, prs,
  projects{name,posture,parked_until,repo,delivery}, in_flight{id,kind,state,repo,name,doing},
...
MODEL=$(printf '%s' "$SNAP" | jq -L "$SCRIPT_DIR" \
  ...
  --argjson candidate_prs "$CANDIDATE_PRS" 'include "fm-project-lifecycle";'"$FM_LANDED_JQ_DEFS"'
  ...
  | ([ $root.backlog.records[]
       ...
       | select(landed_record)
       | select(lifecycle($repo; $projects).archived | not)
       | {id, title, kind, hold_kind, repo:$repo, pr_url, report_path, local_note, completion, home:"(main)", home_id:"(main)"} ]) as $main_done
  ...
  | ($gates_ranked | newest_filed_first | to_entries | sort_by(.value._project_rank, .key) | map(.value)) as $gates_sorted
