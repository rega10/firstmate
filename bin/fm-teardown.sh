...
# The landed-work reachability predicate is owned by bin/fm-work-landed-lib.sh
# (sourced above) so bin/fm-herdr-legacy-repair.sh shares the exact same test.
# This local wrapper keeps teardown's own contract on top of it: the same
# single branch argument, and the PR_URL back-fill that the later backlog-done
# link resolution (BACKLOG_DONE_ARGS) depends on.
work_is_landed() {
  local branch=$1 resolved
  if resolved=$(fm_landed_pr_is_merged "$WT" "$PR_URL" "$branch"); then
    [ -n "$PR_URL" ] || PR_URL=$resolved
    return 0
  fi
...
validate_worktree_teardown_safety() {
  ...
  if [ -n "$unpushed" ] && [ "$MODE" = local-only ]; then
    DEFAULT=$(fm_landed_default_branch "$PROJ") || { echo "REFUSED: cannot determine default branch for $PROJ; expected origin/HEAD, main, or master." >&2; return 1; }
